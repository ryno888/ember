...
namespace Kwerqy\Ember\com\date;
...
class date{
	...
	public static function build($options = []) {
		...
		// date
		if (!$options["date"]) {
			$options["date"] = self::strtodate();
		}

		// build date
		$split = self::split($options["date"]);
    ...
    public static function get_datetime_arr( $start_date, $end_date, $step = '+ 1 hour', $format = false ) {
        ...
        while( $current <= $last ) {
            $dates[] = self::strtodatetime($current, $format);
	...
	public static function get_time_arr() {
		$formatter = function ($time) {
			return self::strtotime("$time:00", "g:i a");
		};
