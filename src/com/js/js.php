		...
		foreach ($options as $option_index => $options_item) {
			...
			switch ($signature) {
				case "*" :
					...
					else $value = "'".\Kwerqy\Ember\com\str\str::escape_singlequote($options_item)."'";
...
//		if ($options["*method"] != "GET" && $options["*form"] === null) $options["*csrf"] = \core::$app->get_response()->get_csrf();
		...
		if($options["form_validate_url"]){
			$JS_string = "{$cid_validate}app.ajax.request_function('{$options["form_validate_url"]}', function(response){
	...
	public static function add_domready_script($event_script) {
  	...
  	public static function add_script($event_script, $options = []) {
		// options
		$options = array_merge([
			"key" => \Kwerqy\Ember\com\str\str::generate_id(),
			"after" => false,
			"top" => false,
		], $options);
	...
	public static function script($js) {
  	...
  	public static function get_script($options = []) {
		...
		$script = self::minify_script($script);

  		if(!$options["wrap"]) return $script;

  		// result
  		return \Kwerqy\Ember\com\ui\ui::make()->tag()->script(["@type" => "text/javascript", "*" => $script]);
  	}
  	//--------------------------------------------------------------------------------
